...
def download_flags(data, download_path="flags"):
    if not os.path.exists(download_path):
        os.makedirs(download_path)
        print(f"Created directory: {download_path}")

    for item in data:
        country_name = item['img'].split('/')[1].split('.')[0]
        img_url = item['img']
        base_url = "" # Add the URL of the instance provided by PicoCTF
        full_url = base_url + img_url
        filename = "".join(x for x in country_name if x.isalnum() or x in " -_") + ".png"
        ...
        try:
            ...
            print(f"Downloaded: {country_name} -> {filepath}")
